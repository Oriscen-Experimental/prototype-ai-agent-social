...
def build_planner_prompt(
    *,
    ...
    return (
        ...
        "\n"
        "**Parameter Inference from Context (IMPORTANT):**\n"
        "Before deciding MISSING_INFO, check if missing parameters can be inferred with HIGH CONFIDENCE from conversation history.\n"
        "Examples (not limited to):\n"
        "- Location: reuse previously specified city unless user mentions a different one\n"
        "- Domain: infer from request (e.g., '找人打球' → person, '找活动' → event)\n"
        "Apply this to ANY parameter that is OBVIOUS from context. When in doubt, ask.\n"
        "If you can infer all required params, use USE_TOOLS instead of MISSING_INFO.\n"
        "\n"
        "**EXCEPTION - Never Infer These (MUST ask user):**\n"
        "- level (skill level): NEVER assume beginner/intermediate/advanced. Always ask.\n"
        "- pace (speed preference): NEVER assume easy/moderate/fast/racing. Always ask.\n"
        "These are personal preferences that vary greatly between users. Always use MISSING_INFO to collect them.\n"
        "\n"
        "→ USE_TOOLS if user wants NEW search/discovery AND all required params available (including inferred ones).\n"
        "Output: {decision, toolName, toolArgs, thought} - NO blocks needed, tool generates UI.\n"
        "→ MISSING_INFO if you want to call a tool BUT required params are truly missing (cannot be inferred).\n"
        ...
        "- start_booking: REQUIRED activity, location, level, pace; OPTIONAL desired_time, headcount (default 3), gender_preference\n"
        "- intelligent_discovery: REQUIRED domain, location; REQUIRED for sports/fitness activities: level, pace\n"
